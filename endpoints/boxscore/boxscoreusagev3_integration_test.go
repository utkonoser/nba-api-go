...
func TestGetBoxScoreUsageV3_Integration(t *testing.T) {
	...
	response, err := client.GetBoxScoreUsageV3(ctx, params)

	if err != nil {
		t.Logf("BoxScoreUsageV3 endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "boxscoreusagev3")
	}

	t.Logf("Successfully fetched boxscoreusagev3 with %d result sets", len(response.ResultSets))
