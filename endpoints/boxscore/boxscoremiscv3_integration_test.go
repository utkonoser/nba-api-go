...
func TestGetBoxScoreMiscV3_Integration(t *testing.T) {
	...
	response, err := client.GetBoxScoreMiscV3(ctx, params)

	if err != nil {
		t.Logf("BoxScoreMiscV3 endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "boxscoremiscv3")
	}

	t.Logf("Successfully fetched boxscoremiscv3 with %d result sets", len(response.ResultSets))
