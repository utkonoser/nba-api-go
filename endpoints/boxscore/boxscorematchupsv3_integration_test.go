...
func TestGetBoxScoreMatchupsV3_Integration(t *testing.T) {
	...
	response, err := client.GetBoxScoreMatchupsV3(ctx, params)

	if err != nil {
		t.Logf("BoxScoreMatchupsV3 endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "boxscorematchupsv3")
	}

	t.Logf("Successfully fetched boxscorematchupsv3 with %d result sets", len(response.ResultSets))
