...
func TestGetBoxScoreAdvancedV3_Integration(t *testing.T) {
	...
	response, err := client.GetBoxScoreAdvancedV3(ctx, params)

	if err != nil {
		t.Logf("BoxScoreAdvancedV3 endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "boxscoreadvancedv3")
	}

	t.Logf("Successfully fetched boxscoreadvancedv3 with %d result sets", len(response.ResultSets))
