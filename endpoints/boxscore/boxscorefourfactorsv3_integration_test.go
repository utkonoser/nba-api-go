...
func TestGetBoxScoreFourFactorsV3_Integration(t *testing.T) {
	...
	if err != nil {
		t.Logf("BoxScoreFourFactorsV3 endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "boxscorefourfactorsv3")
	}

	t.Logf("Successfully fetched boxscorefourfactorsv3 with %d result sets", len(response.ResultSets))
