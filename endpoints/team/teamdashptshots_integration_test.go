...
func TestGetTeamDashPtShots_Integration(t *testing.T) {
	...
	response, err := client.GetTeamDashPtShots(ctx, params)

	if err != nil {
		t.Logf("TeamDashPtShots endpoint error: %v", err)
	...
	if response.Resource != "" {
		assert.Contains(t, response.Resource, "teamdashptshots")
	}

	t.Logf("Successfully fetched teamdashptshots with %d result sets", len(response.ResultSets))
