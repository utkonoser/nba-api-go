package player

import (
	"context"
	"log/slog"
	"net/http"
	"net/http/httptest"
	"os"
	"testing"

	"github.com/utkonoser/nba-api-go/client"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestGetPlayerDashPtShots(t *testing.T) {
	mockResponse := `{
		"resource": "playerdashptshots",
		"parameters": {},
		"resultSets": [
{
				"name": "ClosestDefender10ftPlusShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "ClosestDefenderShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "DribbleShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "GeneralShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "Overall",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "ShotClockShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			},
{
				"name": "TouchTimeShooting",
				"headers": ["PLAYER_ID", "PLAYER_NAME_LAST_FIRST", "SORT_ORDER", "GP", "G"],
				"rowSet": [[1, "Test", "Data", 100, 50]]
			}
		]
	}`

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Contains(t, r.URL.Path, "playerdashptshots")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(mockResponse))
	}))
	defer server.Close()

	logger := slog.New(slog.NewTextHandler(os.Stdout, nil))
	httpClient := client.NewHTTPClient(server.URL+"/%s", DefaultHeaders(), logger)
	c := &Client{
		httpClient: httpClient,
		logger:     logger,
	}

	params := PlayerDashPtShotsParams{}

	response, err := c.GetPlayerDashPtShots(context.Background(), params)

	require.NoError(t, err)
	require.NotNil(t, response)
	assert.Equal(t, "playerdashptshots", response.Resource)
	// Note: ResultSets may be empty for some endpoints
	assert.NotNil(t, response.ResultSets)
}
